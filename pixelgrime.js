...
rvs2 = reverbStereo = ( input, len = 16e3, vibratoSpeed = [7,9], dry = .4, wet = 1, dsp = 3, lerpx=4, highpass=.03, lowpass = .5, compThresh = 64, compAtk = 9, compRel = 9, compRatio = 64, vibratoDepth = 99, t2 ) => (
	...
	F[ I + vcs + 1 + ( (T % len) / dsp )|0 ] = lp2(
		o.reduce((a,e,i)=> a = (a+e)/compRatio + lim2(
...
rvs = reverbStereo = ( input, len = 16e3, vibratoSpeed = [7,9], dry = .4, wet = 1, dsp = 3, lerpx=4, highpass=.03, lowpass = .5, compThresh = 64, compAtk = 9, compRel = 9, compRatio = 64, vibratoDepth = 99, t2 ) => (
	vcs = vibratoSpeed.length,
	t2 ??= r(vcs, T ), //array of all T the same size as vibratoSpeed[], could also be T/2 if specified in args
	o=[],//out=[0,0] //can reuse o
	t2.map( (t2val,i) => (
		...
		o[i] = hp(
			input * dry/2 + wet*300/compThresh/vcs * seq(
				F, 0, I + vcs-i + ( t2val % len ) / dsp, lerpx //more accurate but removing isn't a problem
				//F, 0, I + ( t2val % len ) / dsp, lerpx
			) || 0
			, highpass
		)
	...
	F[ I + 1 + ( (T % len) / dsp )|0 ] = lp2(
		o.reduce((a,e,i)=> a = tanh(
			(a+e)/compThresh/(1+i/2)
		)*compThresh*(1+i/2)
	), lowpass ),
	I += 0|(len / dsp),
	o.map((e,i)=>o[i%2]+=e),o //first 2 voices will be double volume
...
//vs=r(16,3).map((e,i)=>e*1.618**(i/2)),
//vs=r(16,399).map((e,i)=>e/1.618**i),
vs=r(16,250).map((e,i)=>e/1.618**(i/2)),
...
V = rvs( vl * (A1/3 + L2[0]/2) + L3, 8e3, vs, .1, t<2?3:1, 4, 0, .1, .7, 69, 99, 9, 64, 399 ),
//V = rvs( vl * (A1/3 + L2[0]/2) + L3, 8e3, vs, .1, t<9?3:1.2, 4, 0, .1, .5, 9, .1, 64, 99 ),
