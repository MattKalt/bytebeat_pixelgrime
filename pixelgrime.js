...
rvs = reverbStereo = ( input, len = 16e3, vibratoSpeed = [1,2], wet = .7, dry = .4, feedb = 1, dsp = 3, lerpx=4, highpass=.03, lowpass = .5, compSpeed = .1, compThresh = 64, t2 = [T,T], vibratoDepth = 99 ) => (
	...
	F[ I + ( (T % len) / dsp )|0 ] = lp2( o.reduce((a,e)=>a+=e), lowpass ),
	I += 0|(len / dsp),
...
//V = rvs( vl * (A1/3 + L2[0]/2) + 2 * L3, 8e3, vv, seq(fb,18), .1, .4, 4, 0, .1, .1, 9, [T%3e3,T%5e3,T%7e3,T/2,T], 99 ),

V = rvs( vl * (A1/3 + L2[0]/2) + L3, 3e3, vv, seq(fb,18), .1, .4, 4, 0, .1, .5, .1, min(9,5+t/2e4), [T,T,T,T,T], 99 ),
...
//		reverb
lp2( V[ch], min(1,t/2e5+.1)) * min(1.3,t/6e5+.3) * 2

//, 1, 512, 1, 150 ),
,.001)/max(ch?171:256,30-t/8e3))*(ch?1:1.5),
//,.001)*.01/max(ch?171:256,30-t/8e3))*(ch?1:1.5)*99,
